...
training.py
-----------
Training routines for Q-learning agents in 4x4 connect-4 Tic-Tac-Toe.
...
def _reward_for_player(winner: Optional[int], player: int) -> float:
    """Return terminal reward from a specific player's perspective."""
    if winner is None:
        return 0.0
    if winner == 0:
        return 0.5
...
def _run_episode_selfplay(
    ...
    while True:
        ...
        winner = info["winner"]
        agent_reward = _reward_for_player(winner, current_player) if done else 0.0
        ...
        if last_exp[other_player] is not None:
            ...
            other_reward = (_reward_for_player(winner, other_player)
                            if done else 0.0)
            if done:
                td = other_agent.update(ls, la, other_reward,
                                        next_state, [], True)
            else:
...
def train_vs_random(
    ...
    for ep in range(total_episodes):
        ...
        if (ep + 1) % eval_interval == 0:
            ...
            if verbose:
                print(
                    f"[Ep {ep+1:>7d}]  eps={agent.epsilon:.4f}  "
                    f"avg_r={avg_r:+.3f}  "
...
def train_selfplay(
    ...
    for ep in range(total_episodes):
        ...
        if (ep + 1) % eval_interval == 0:
            ...
            if verbose:
                print(
                    f"[Ep {ep+1:>7d}]  eps={agent1.epsilon:.4f}  "
                    f"avg_r={avg_r:+.3f}  "
                    f"win={wins/eval_episodes:.2%}  "
                    f"draw={draws/eval_episodes:.2%}"
                )
